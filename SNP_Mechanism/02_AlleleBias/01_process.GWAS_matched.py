...
def matchSNP():
    ...
    matchDict={}
    output1=open(DIR+'/MatchList.GWAS_matched.txt','w')
	...
	for i in intersect:
	    ...
	    [ref, alt, genotype, depth]=infoDict[sampleID][i]
	    newLine=[sampleID, chrID, pt, ref, alt, genotype]
...
def collectReads():
	...
	if matchDict.has_key(sampleID):
		...
		runLine='samtools view '+BAM_DIR+'/'+sampleID+'.H3K27ac.sorted.nodup.filtered.bam \''+snpID+'\' > '+DIR+'/SNP_Reads/'+sampleID+'.'+snpID.replace(':','_').replace('-','_')+'.GWAS_matched.txt'
		print runLine
		os.system(runLine)
...
def collectSeq():
    # load query allele
    queryDict={}
    input1=open(DIR+'/MatchList.GWAS_matched.txt','r')
    all_input1=input1.readlines()
    for line in all_input1[1:]:
	each=line.strip().split('\t')
	sampleID=each[0]
    ...
    #
    output1=open(DIR+'/AllelicReads.GWAS_matched.txt','w')
	...
	pt1=str(query_pt-1)
	pt2=str(query_pt+1)
	input1=open('SNP_Reads/'+sampleID+'.'+chrID+'_'+pt1+'_'+pt2+'.GWAS_matched.txt','r')
	all_input1=input1.readlines()
	countDict={}
	for line in all_input1:
	    each=line.strip().split('\t')
...
def calcBinomP():
    ...
    input1=open(DIR+'/AllelicReads.GWAS_matched.txt', 'r')
    all_input1=input1.readlines()
    output1=open(DIR+'/AllelicReads.GWAS_matched.pval.txt','w')
	...
	log2Enrich= numpy.log2(float(altReads) / altExp)
	binomP = stats.binom_test(x=altReads, n=allReads, p=0.5)

	if binomP < 0.05:
	    if int(refReads) > int(altReads):
		label= 'refEnrich'
	    else:
		label= 'altEnrich'
	else:
