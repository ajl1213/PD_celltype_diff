...
def intersect():
	...
	for i in dataTypeList:
	    run_line='intersectBed -a '+DIR+'/BED/BrainSN.sigEQTL.bed -b '+dysPeak_DIR+'/'+i+'.'+cellType+'.bed -wa -wb > '+DIR+'/BED/'+i+'.'+cellType+'.peakIntersect.bed'
	    print run_line
	    os.system(run_line)
...
def makeData():
	...
	for i in dataTypeList:
	    totalSize=0
	    input1=open(dysPeak_DIR+'/'+i+'.'+cellType+'.bed','r')
	    all_input1=input1.readlines()
	    for line in all_input1:
		each=line.strip().split('\t')
	    ...
	    peakFracDict[cellType+';'+i]=celltype_frac
    ...
    output1=open(DIR+'/sigEQTL.dys_cRE_asso.txt','w')
    output1.write('CellType\tDataType\tnEQTL\ttotalEqtl\texpected\tlog2Enrich\tPval\n')
    for cellType in cellTypeList:
	for i in dataTypeList:
	    eqtlList=[]
	    input1=open(DIR+'/BED/'+i+'.'+cellType+'.peakIntersect.bed','r')
	    all_input1=input1.readlines()
	    for line in all_input1:
		each=line.strip().split('\t')
		chrID=each[0]
	    ...
	    nExp=float(len(totalEqtlList)) * peakFracDict[cellType+';'+i]

	    log2Enrich=numpy.log2(nEqtl/nExp)
	    #binomP=stats.binom_test(x=nEqtl, n=float(len(totalEqtlList)), p=peakFracDict[cellType+';'+i])
	    ...
	    newLine=[cellType, i, str(nEqtl), str(len(totalEqtlList)), str(nExp), str(log2Enrich), str(fisherP)]
	    output1.write('\t'.join(newLine)+'\n')
