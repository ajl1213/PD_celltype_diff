...
snDegFile='/home/ajl1213/Projects/PD/NeuronJ/CellTypeDEGs/snRNA.DEG_list.txt'
bulkDegFile='/home/ajl1213/Projects/PD/NeuronJ/DiffAnal/01_RNA/1_BulkDEG/BulkDEG.funcCell.txt'
...
os.system('mkdir '+DIR+'/RandDegPermute')
...
def makeDegList():
	...
	for cellType in cellTypeList:
		...
		input1=open(bulkDegFile,'r')
		all_input1=input1.readlines()
		output1=open(DIR+'/DegList/'+dataType+'.'+cellType+'.'+winDist+'.bulkDEG.bed','w')
		for line in all_input1[1:]:
		    each=line.strip().split('\t')
		    geneID=each[0]
		    if each[1] == dataType:
			if cellType in each[2].split(','):
			    tss=tssDict[geneID]
			    chrID=tss.split(':')[0]
			    pt=int(tss.split(':')[1])
	...
	for cellType in cellTypeList:
	    for winDist in windowDict:
		input1=open(snDegFile,'r')
		all_input1=input1.readlines()
		output1=open(DIR+'/DegList/'+dataType+'.'+cellType+'.'+winDist+'.snDEG.bed','w')
		for line in all_input1[1:]:
		    each=line.strip().split('\t')
		    geneID=each[2]
		    if each[1] == dataType:
...
def makeScript():
	...
	for cellType in cellTypeList:
		...
		output1=open('PBS_randDegPermute.'+dataType+'.'+cellType+'.'+winDist+'.pbs','w')
		output1.write('#PBS -N randDegPermute.'+dataType+'.'+cellType+'.'+winDist+'\n')
		...
		output1.write('python '+DIR+'/1b_permuteRandDEG.py '+dataType+' '+cellType+' '+winDist+' '+PeakDIR+' '+str(maxIter)+' '+gtf+'\n')
		...
		os.system('qsub PBS_randDegPermute.'+dataType+'.'+cellType+'.'+winDist+'.pbs')
