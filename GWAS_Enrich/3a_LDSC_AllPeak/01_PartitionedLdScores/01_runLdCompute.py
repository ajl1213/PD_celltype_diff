...
def makeAnnoFile():
	...
	for chrID in chrList:
	    output1=open('PBS_makeAnno.'+cellType+'.'+chrID+'.pbs','w')
	    output1.write('#PBS -N makeAnno.'+cellType+'.'+chrID+'\n')
	    ...
	    output1.write(PythonDIR+'/python '+ScriptDIR+'/make_annot.py --bed-file '+PeakDIR+'/'+cellType+'.atacPeak.bed --bimfile 1000G_EUR_Phase3_plink/1000G.EUR.QC.'+chrID+'.bim --annot-file '+DIR+'/AnnoLdFile/'+cellType+'.'+chrID+'.anno.gz\n')
	    output1.write('sleep 30\n')
	    output1.write('exit 0')
	    output1.close()

	    os.system('qsub PBS_makeAnno.'+cellType+'.'+chrID+'.pbs')
...
def ComputeLD():
    ...
    for cellType in cellTypeList:
	for chrID in chrList:
	    output1=open('PBS_LdCompute.'+cellType+'.'+chrID+'.pbs','w')
	    output1.write('#PBS -N LdCompute.'+cellType+'.'+chrID+'\n')
	    output1.write('#PBS -q workq\n')
	    output1.write('#PBS -l nodes=1:ppn='+str(ncpu)+'\n')
	    output1.write('#PBS -j oe\n')
	    output1.write('\n')
	    ...
	    output1.write(PythonDIR+'/python '+ScriptDIR+'/ldsc.py --l2 --bfile 1000G_EUR_Phase3_plink/1000G.EUR.QC.'+chrID+' --ld-wind-cm 1 --annot '+DIR+'/AnnoLdFile/'+cellType+'.'+chrID+'.anno.gz --thin-annot --out '+DIR+'/AnnoLdFile/'+cellType+'.'+chrID+' --print-snps hapmap3_snps/hm.'+chrID+'.snp\n')
	    ...
	    os.system('qsub PBS_LdCompute.'+cellType+'.'+chrID+'.pbs')
