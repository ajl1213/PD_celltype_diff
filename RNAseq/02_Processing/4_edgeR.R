...
data1 <- as.data.frame(read.table('/home/ajl1213/Projects/PD/data/RNAseq/2_Cellularity/AdjustedBulk/RNA.cellAdjusted.combat.qq.txt', header=T, row.names=1))
...
# Estimate dispersion
y_nd <- estimateDisp(y_n, design, robust=TRUE)
...
##
df_final=data.frame(geneID=row.names(qlf$table), qlf$table, adjPval)
write.table(df_final, file=paste0('ValTable/RNA.valTable.txt'), row.names=F, col.names=T, sep="\t", quote=F)
